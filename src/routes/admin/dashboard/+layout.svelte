...
<style>
  .admin-layout {
    ...
    min-height: 100vh;
    min-height: 0;
    width: 100%; /* don't force 100vw which can cause horizontal scroll on mobile */
    background: #f4f6fb;
    overflow-x: hidden; /* prevent accidental horizontal overflow from wide children */
  }
  .sidebar {
    width: 280px;
  ...
  .dashboard-main {
    ...
    padding: 0;
    background: #f8fafc;
    min-height: 100vh; /* allow content to grow naturally on small screens */
    display: flex;
    flex-direction: column;
  ...
  /* Only offset main when sidebar is open (sidebar is fixed) so content doesn't get hidden */
  .admin-layout.sidebar-open .dashboard-main {
    margin-left: 280px;
  }
  @media (max-width: 900px) {
    ...
    .admin-layout.sidebar-open .top-toggle {
      display: none;
    }
    .dashboard-main {
      padding: 0;
      ...
      overflow-y: auto;
      background: #f8fafc;
      margin-left: 0; /* remove left margin on small screens where sidebar is overlay */
    }
  }
</style>