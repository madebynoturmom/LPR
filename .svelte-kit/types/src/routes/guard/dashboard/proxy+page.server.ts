...
import { guestPass as guestPassTable, guestPassHistory, eventLog } from '$lib/server/db/schema';
...
export const load = async ({ locals }: Parameters<PageServerLoad>[0]) => {
  ...
  const allVisitors = await db.select().from(guestPassTable).where(eq(guestPassTable.type, 'visitors'));
  ...
  const allFood = await db.select().from(guestPassTable).where(eq(guestPassTable.type, 'food_delivery'));
...
export const actions = {
  admit: async ({ request, locals }: import('./$types').RequestEvent) => {
  const check = requireGuard(locals);
  if (!check.ok) return fail(check.status ?? 403, { error: check.message });
    ...
    try {
      ...
      await db.insert(eventLog).values({ id: `E${Date.now()}`, type: 'vehicle_entry', userId: guardUser.id, details: `Admitted pass ${pass.id} (${pass.plateNumber}) by guard ${guardUser.username}`, timestamp: new Date() });
      return { success: true };
    } catch (e) {
      return fail(500, { error: 'Failed to admit pass.' });
    }
  },
  deny: async ({ request, locals }: import('./$types').RequestEvent) => {
  const check = requireGuard(locals);
  if (!check.ok) return fail(check.status ?? 403, { error: check.message });
  ...
  const pass = await db.select().from(guestPassTable).where(eq(guestPassTable.id, id)).get();
    ...
    try {
      ...
      await db.insert(guestPassHistory).values({ id: pass.id, plateNumber: pass.plateNumber, visitTime: new Date(pass.visitTime), durationMinutes: pass.durationMinutes, status: 'revoked', userId: pass.userId, type: pass.type, revokedAt: new Date(), name: pass.name, phone: pass.phone });
      await db.delete(guestPassTable).where(eq(guestPassTable.id, id));
      const guardUser = check.user;
      await db.insert(eventLog).values({ id: `E${Date.now()}`, type: 'access_denied', userId: guardUser.id, details: `Denied pass ${pass.id} (${pass.plateNumber}) by guard ${guardUser.username}`, timestamp: new Date() });
      return { success: true };
    } catch (e) {
